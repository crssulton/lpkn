            ...
            <div>
		        ...
		        <div className="wrapper wrapper-content">
												            ...
												            <td style={{'width' : '20%'}}>
												            	<center>
												            		<select
												            			...
												            			value={daftar.kehadiran.find(x => x.jadwal == this.state.jadwal.id).status}
												            			onChange={(e) => {
												            				const self = this
												            				let newKehadiran = {}
												            				...
												            				let idKehadiran = daftar.kehadiran.find(x => x.jadwal == this.state.jadwal.id).id
												            				console.log(newKehadiran)
												            				fetch(BASE_URL + '/api/kehadiran/' + idKehadiran + '/', {
																				method: 'patch',
																				headers: {
																					'Authorization': 'JWT ' + window.sessionStorage.getItem('token'),
																				...
																				if (response.status == 200) {
																					...
																					toastr.warning("Data gagal diubah", "Gagal ! ")
																				}
																			}).then(function(data) {
                    ...
                    <div id="print_data" style={{'backgroundColor': 'white', 'display' : 'none'}}>
                    	<table>
                    		<tr>
                    			<td style={{'width' : '60%'}}>Mata Kuliah </td>
                    			<td style={{'width' : '100%'}}>: {this.state.jadwal.mata_kuliah_info.nama}</td>
                    		</tr>
                    		<tr>
                    			<td>Tanggal </td>
                    			<td>: {this.state.jadwal.start}	</td>
